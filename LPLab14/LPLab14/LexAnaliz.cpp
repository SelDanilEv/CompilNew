...
namespace LexA
{
	...
	void analyze(int currentLine, char *fulltextch)                         //функция анализа
	{
		...
		IT::IdTable myidtable = IT::Create(amountOfLex);                         //создания таблиц
		LT::LexTable mylextable = LT::Create(amountOfLex);
		...
		for (int i = 0; i < amountOfLex; i++) {
			...
			if (identifyLex == 0 || identifyLex == 16 || identifyLex == 10 || identifyLex == 11 || identifyLex == 7 )          //если идентификатор
			{
				...
				case 10:                                                         //область видимости
					...
					bufferi = mylextable.size;
					while (mylextable.table[bufferi].lexema != LEX_LEFTTHESIS && mylextable.table[bufferi].lexema != LEX_MAIN&& mylextable.table[bufferi].lexema != LEX_SEMICOLON)
					{
						if (mylextable.table[bufferi].lexema == LEX_ID)
							myidtable.table[mylextable.table[bufferi].idxTI].areaOfVisibility[counterOfAreaOfVisibility] = counterOfBracket;
						bufferi--;
					}
					break;
				case 11:
				...
				case 16:                                                             //литералы
					...
					IT::Add(myidtable, myentryI);                        //добавить в IT
					myentryL.idxTI = myidtable.size - 1;
					break;
				case 0:
					if (IT::IsId(myidtable, buff_name) == TI_NULLIDX && IT::IsId(myidtable, buff_name_str) == TI_NULLIDX)      //было ли уже в таблице идентификаторов   
					{
						addNewInIT(myidtable, mylextable);
					}
					else
					{
						...
						{
							bufferb = true;
							for (int y = 0; y < myidtable.size; y++)
							{
								bufferi = 0;
								bufferi1 = 1;
								buffer = "";
								for (int w = 0; w < 5; w++)
									buffer += myidtable.table[y].id[w];
								if (std::strcmp(str.c_str(), buffer.c_str()) == 0)        //если названия сошлись
								{
									while (myidtable.table[y].areaOfVisibility[bufferi1] != 0)  //узнаю сколько значащих в массиве видимости подозреваемого
									...
									if (bufferi1 == counterOfAreaOfVisibility + 1) {                  //если одинокого цифр 
										if (myidtable.table[y].areaOfVisibility[bufferi1 - 1] == areaOfVisibilityLexAnaliz[bufferi1 - 1]) //запрет если одинаковык
											bufferb = false;
									}
									else {
										for (int q = 0; q < counterOfAreaOfVisibility + 1; q++)
											if (myidtable.table[y].areaOfVisibility[q] != areaOfVisibilityLexAnaliz[q])
												bufferb = false;
									}
								}
							}
							if (bufferb)
								addNewInIT(myidtable, mylextable);
							LexInIT = IT::IsIdWithAreaOfVisibility(myidtable, buff_name, areaOfVisibilityLexAnaliz);
							myentryL.idxTI = LexInIT;
						}
						else {
							LexInIT = IT::IsIdWithAreaOfVisibility(myidtable, buff_name_str, areaOfVisibilityLexAnaliz);
							myentryL.idxTI = LexInIT;
						}
					}
				}
		...
		polishNotation(32,mylextable,myidtable);
		//polishNotation(62,mylextable,myidtable);

		std::ofstream fileLT;               //формрирование файлов таблиц
		fileLT.open("LT.txt");
		LT::showTable(mylextable, fileLT);
		fileLT.close();

		std::ofstream fileLT_mini;
		fileLT_mini.open("LT_mini.txt");
		LT::showTable_mini(mylextable, fileLT_mini);
		fileLT_mini.close();

		std::ofstream fileIT;
		fileIT.open("IT.txt");
		IT::showTable(myidtable, fileIT);
		fileIT.close();
		system("pause");
	}
}